...
from xtrimomultimer.data.data_transforms import (
    make_seq_mask,
...
@pytest.mark.parametrize(
    "seq",
    [
        torch.tensor([range(20)], dtype=torch.int64).transpose(0, 1),
    ],
)
def test_make_seq_mask(seq):
...
def test_add_distillation_flag():
    protein = {}
...
def test_make_all_atom_aatype(seq):
    seq_one_hot = torch.FloatTensor(seq.shape[0], 20).zero_()
    seq_one_hot.scatter_(1, seq, 1)
    protein_aatype = seq_one_hot.clone().detach()
    protein = {"aatype": protein_aatype}
...
@pytest.mark.parametrize(
    "template_seq",
    [
        torch.tensor(list(range(20)) * 2, dtype=torch.int64),
    ],
)
def test_fix_templates_aatype(template_seq):
...
def test_correct_msa_restypes():
    ...
    assert torch.all(
        torch.eq(
            torch.tensor(features["msa"].shape), torch.tensor(protein["msa"].shape)
        )
    )


def test_squeeze_features():
    with open("tests/test_data/features.pkl", "rb") as file:
        features = pickle.load(file)
...
def test_randomly_replace_msa_with_unknown():
    ...
    unknown_proportion_in_msa = torch.bincount(protein["msa"].flatten()) / torch.numel(
        protein["msa"]
    )
...
@pytest.mark.parametrize(
    "protein",
    [
        {"extra_msa": torch.rand((512, 100, 23))},
    ],
)
def test_delete_extra_msa(protein):
...
def test_make_masked_msa():
    ...
    protein = make_masked_msa.__wrapped__(
        protein, masked_msa_config, replace_fraction=0.15
    )
...
def test_make_msa_feat():
    with open("tests/test_data/features.pkl", "rb") as file:
        features = pickle.load(file)

    protein = {
        "between_segment_residues": torch.tensor(features["between_segment_residues"]),
        "msa": torch.tensor(features["msa"], dtype=torch.int64),
...
def test_crop_templates():
    with gzip.open("tests/test_data/sample_feats.pickle.gz", "rb") as file:
        features = pickle.load(file)

    protein = {
        "template_aatype": torch.tensor(features["true_msa"][0]),
