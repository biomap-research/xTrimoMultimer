...
import torch
from xtrimomultimer.model_acc.kernel import LayerNorm as FastLayerNorm
...
def test_layernorm():
    ...
    for shape in test_shape:
        for dtype in test_dtype:
            ...
            torch_module = torch.nn.LayerNorm(normalized_shape=dim_).to(
                device=test_device, dtype=dtype
            )
            fastnn_module = FastLayerNorm(normalized_shape=dim_).to(
                device=test_device, dtype=dtype
            )
            ...
            backward_weight_error = (
                torch.max(
                    torch.abs(torch_module.weight.grad - fastnn_module.weight.grad)
                )
                .cpu()
                .item()
            )
            assert (
                backward_weight_error < tolerance_eps[dtype]
            ), f"Error when {shape} {dtype}"

            backward_bias_error = (
                torch.max(torch.abs(torch_module.bias.grad - fastnn_module.bias.grad))
                .cpu()
                .item()
            )
            assert (
                backward_bias_error < tolerance_eps[dtype]
            ), f"Error when {shape} {dtype}"
