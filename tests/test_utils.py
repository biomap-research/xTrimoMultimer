...
from xtrimomultimer.utils.rigid_utils import (
    Rotation,
    Rigid,
    quat_to_rot,
    rot_to_quat,
)
from xtrimomultimer.utils.tensor_utils import chunk_layer, _chunk_slice
...
def test_chunk_layer_dict():
    ...
    assert torch.all(chunked["out"] == unchunked["out"])
    assert torch.all(chunked["inner"]["out"] == unchunked["inner"]["out"])


def test_chunk_slice_dict():
    ...
    for i in range(prod):
        for j in range(i + 1, prod + 1):
            ...
            assert torch.all(chunked == chunked_flattened)
