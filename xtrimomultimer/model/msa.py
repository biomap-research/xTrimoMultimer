...
from xtrimomultimer.utils.checkpointing import get_checkpoint_fn
...
class MSAAttention(nn.Module):
    ...
    def forward(
        ...
        if msa_att_row:
            ...
            z:[1, no_head, N_res, N_res]
        else msa_att_col:
            m:[N_res, N_seq, C_m]
            mask_bias:[N_res, 1, 1, N_seq]
        """
        m, mask_bias, z = self._prep_inputs(m, z, mask)
        ...
        if z is not None:
            """
            if msa_att_row:
                biases:[[N_seq, 1, 1, N_res], [1, no_head, N_res, N_res]]
            else msa_att_col:
                biases:[[N_res, 1, 1, N_seq]]
            """
            biases.append(z)
...
class MSARowAttentionWithPairBias(MSAAttention):
    ...
    def __init__(
        self, c_m: int, c_z: int, c_hidden: int, no_heads: int, inf: float = 1e9
    ):
        """
        Args:
            c_m:
...
class MSAColumnAttention(nn.Module):
    ...
    def __init__(self, c_m: int, c_hidden: int, no_heads: int, inf: float = 1e9):
        """
        Args:
            c_m:
