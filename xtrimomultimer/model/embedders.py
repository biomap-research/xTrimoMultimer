...
from xtrimomultimer.utils import atom_representations
...
from xtrimomultimer.utils.tensor_utils import one_hot

from xtrimomultimer.model.template import (
    TemplatePairStack,
    TemplatePointwiseAttention,
)
from xtrimomultimer.utils import geometry
from xtrimomultimer.utils.tensor_utils import one_hot, tensor_tree_map, dict_multimap
...
class TemplateEmbedder(nn.Module):
    def __init__(self, config):
        ...
        self.template_pair_stack = TemplatePairStack(
            **config["template_pair_stack"], is_multimer=False
        )
...
class TemplateSingleEmbedderMultimer(nn.Module):
    ...
    def forward(
        ...
        (
            template_chi_angles,
            template_chi_mask,
        ) = atom_representations.compute_chi_angles(
            atom_pos,
...
class TemplateEmbedderMultimer(nn.Module):
    def __init__(self, config):
        ...
        self.template_pair_stack = TemplatePairStack(
            **config["template_pair_stack"], is_multimer=True
        )
    ...
    def forward(
        ...
        for i in range(n_templ):
            ...
            rigid, backbone_mask = atom_representations.make_backbone_affine(
                atom_pos,
