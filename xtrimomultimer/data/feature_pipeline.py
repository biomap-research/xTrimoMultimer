...
from xtrimomultimer.data import input_pipeline_base as input_pipeline
...
def np_example_to_features(
    ...
    with torch.no_grad():
        features = input_pipeline.process_tensors_from_config(
            tensor_dict, cfg.common, cfg[mode], is_multimer
        )
...
class FeaturePipeline:
    ...
    def process_features(
        self, raw_features: FeatureDict, is_multimer: bool, mode: str = "train"
    ) -> FeatureDict:
        if mode != "predict":
            raise ValueError("Training is not currently available")
        else:
            processed_feature = np_example_to_features(
