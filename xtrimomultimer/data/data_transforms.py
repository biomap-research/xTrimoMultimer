...
from xtrimomultimer.config import NUM_RES, NUM_EXTRA_SEQ, NUM_TEMPLATES, NUM_MSA_SEQ
from xtrimomultimer.np import residue_constants as rc
from xtrimomultimer.utils.rigid_utils import Rotation, Rigid
...
def fix_templates_aatype(protein):
    ...
    if num_templates > 0:
        ...
        new_order = torch.tensor(
            new_order_list,
            dtype=torch.int64,
            device=protein["template_aatype"].device,
...
def make_masked_msa(protein, config, replace_fraction):
    ...
    random_aa = torch.tensor(
        [0.05] * 20 + [0.0, 0.0], dtype=torch.float32, device=protein["aatype"].device
    )
...
def make_atom14_masks_np(batch):
    batch = tree_map(
        lambda n: torch.tensor(n, device=batch["aatype"].device), batch, np.ndarray
    )
