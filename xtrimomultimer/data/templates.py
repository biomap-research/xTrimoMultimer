...
from xtrimomultimer.data import parsers, mmcif_parsing
from xtrimomultimer.data.errors import Error
from xtrimomultimer.data.tools import kalign
...
def make_empty_template(query_sequence, is_multimer):
    # todo, support multimer later.
    num_res = len(query_sequence)
    # deepmind's setting
    # template_features = {
    #     'template_aatype': np.zeros((1, num_res, len(residue_constants.restypes_with_x_and_gap)),
    #                                 np.float32),
    ...
    template_features = (
        {
            ...
            "template_all_atom_positions": np.zeros(
                (0, num_res, residue_constants.atom_type_num, 3)
            ).astype(np.float32),
            "template_sum_probs": np.zeros((0, 1)).astype(np.float32),
            "template_all_atom_mask": np.zeros(
                (0, num_res, residue_constants.atom_type_num)
            ).astype(np.float32),
        }
        if not is_multimer
        else {
            "template_aatype": np.zeros((1, num_res, 22)).astype(np.int64),
    ...
    for f in os.listdir(mmcif_dir):
        if f.endswith(".cif"):
            ...
            if mmcif.mmcif_object is None:
                logger.info(f"Failed to parse {f}. Skipping...")
                continue
...
def _assess_hhsearch_hit(
    ...
    if align_ratio <= min_align_ratio:
        raise AlignRatioError(
            f"Proportion of residues aligned to query too small. Align ratio: {align_ratio}."
        )
...
def _find_template_in_pdb(
    ...
    if chain_sequence and (template_sequence in chain_sequence):
        ...
        logger.info(
            f"Found an exact template match {pdb_id}_{template_chain_id}. mapping_offset={mapping_offset}."
        )
...
def _realign_pdb_template_to_query(
    ...
    try:
        ...
        (old_aligned_template, new_aligned_template) = msa.sequences
...
def _extract_template_features(
    ...
    try:
        ...
        logger.info(
            f"Sequence in {pdb_id}_{chain_id}: {template_sequence} successfully realigned to {seqres}"
        )
    ...
    mask_name = "template_all_atom_masks" if is_multimer else "template_all_atom_mask"
    return (
        {
            "template_all_atom_positions": np.array(templates_all_atom_positions),
            mask_name: np.array(templates_all_atom_masks),
...
def _process_single_hit(
    ...
    comment: be almost consistent with deepmind's v2.2.0.
    """
    # Fail hard if we can't get the PDB ID and chain name from the hit.
    hit_pdb_code, hit_chain_id = _get_pdb_id_and_chain(hit)

    # new in v2.2.0, This hit has been removed (obsoleted) from PDB, skip it.
    ...
    try:
        features, realign_warning = _extract_template_features(
            ...
            kalign_binary_path=kalign_binary_path,
            _zero_center_positions=_zero_center_positions,  # new variable in openfold.
        ...
        warning = (
            "%s_%s (sum_probs: %.2f, rank: %d): feature extracting errors: %s, mmCIF parsing errors: %s"
            % (
                hit_pdb_code,
                hit_chain_id,
                hit.sum_probs,
...
class HhsearchHitFeaturizer(TemplateHitFeaturizer):
    def get_templates(
        ...
        for template_feature_name in TEMPLATE_FEATURES:
            if is_multimer and template_feature_name == "template_all_atom_mask":
                template_feature_name = "template_all_atom_masks"
            template_features[template_feature_name] = []

        # different from deepmind's implementation.
        # Always use a max_template_date. Set to query_release_date minus 60 days if that's earlier.
        ...
        if self._shuffle_top_k_prefiltered:  # different from deepmind's implementation.
        ...
        for i in idx:
            ...
            if result.features is None:
                ...
                if is_multimer:
                    # different for multimer and monomer
                    # see details: https://github.com/deepmind/alphafold/blob/v2.2.0/alphafold/data/templates.py#L982
                    already_seen_key = result.features["template_sequence"]
                    if already_seen_key in already_seen:
                        logger.warning(f"already_seen_key={already_seen_key}")
                        continue
        ...
        logger.info(
            f"tot hits: {len(hits)}, remained (after filter): {len(filtered)}, final: {num_hits}"
        )
        if num_hits == 0:
            logger.warning("there is no available templates.")
            template_features = make_empty_template(query_sequence, is_multimer)
        else:
            for name in template_features:
                if is_multimer and name == "template_all_atom_masks":
                    dtype = TEMPLATE_FEATURES["template_all_atom_mask"]
                else:
...
class HmmsearchHitFeaturizer(TemplateHitFeaturizer):
    def get_templates(
        ...
        filtered = list(
            sorted(
                filtered,
                key=lambda x: x.sum_probs if x.sum_probs else 0.0,
                reverse=True,
            )
        )
        idx = list(range(len(filtered)))
        ...
        if already_seen:
            ...
            if is_multimer:
                template_features["template_all_atom_masks"] = np.zeros(
                    (1, num_res, residue_constants.atom_type_num), np.float32
                )
            else:
                template_features["template_all_atom_mask"] = np.zeros(
                    (1, num_res, residue_constants.atom_type_num), np.float32
                )
