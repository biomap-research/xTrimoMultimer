...
from xtrimomultimer.np import protein, residue_constants
import xtrimomultimer.utils.loss as loss
from xtrimomultimer.np.relax import cleanup, utils
...
def _openmm_minimize(
    ...
    amber_device = "CUDA" if use_gpu else "CPU"
    logger.info(f"Using {amber_device} as amber backend")
    integrator = openmm.LangevinIntegrator(0, 0.01, 0.0)
    platform = openmm.Platform.getPlatformByName(amber_device)
...
def _run_one_iteration(
    *,
    ...
    while not minimized and attempts < max_attempts:
        attempts += 1
        try:
            logger.info("Minimizing protein, attempt %d of %d.", attempts, max_attempts)
            ...
            # todo, there is potential bug in _openmm_minimize when running on CUDA
            logger.info(e)
            break
    if not minimized:
...
def get_initial_energies(
    ...
    simulation = openmm_app.Simulation(
        ...
        openmm.Platform.getPlatformByName(amber_device),
    )
    energies = []
    for pdb in openmm_pdbs:
        try:
            ...
            logger.error("Error getting initial energy, returning large value %s", e)
