...
def _process_translations_dict(
    d: Dict[str, Any], top_layer: bool = True
) -> Dict[str, Any]:
    flat = {}
    for k, v in d.items():
        if type(v) == dict:
...
def stacked(param_dict_list: List[Dict[str, Any]], out=None) -> Dict[str, Any]:
    """
    Args:
        param_dict_list:
...
def assign(translation_dict: Dict[str, Any], orig_weights):
...
def import_jax_weights_(
    model, npz_path: Union[str, bytes, PathLike], version: str = "model_1"
):
