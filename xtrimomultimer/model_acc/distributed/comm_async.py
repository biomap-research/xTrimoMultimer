...
class GatherAsync(torch.autograd.Function):
    ...
    @staticmethod
    def backward(
        ctx: "GatherAsync", grad_output: Tensor, grad_work=None
    ) -> Tuple[Tensor]:
        if ctx.dim == 2:
            mp_size = gpc.get_world_size(ParallelMode.TENSOR)
            n, h, w, c = grad_output.shape
...
class All_to_All_Async(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx: "All_to_All_Async", input_: Tensor, in_dim: int = -1, out_dim: int = -1
    ) -> Tensor:
        ctx.in_dim = in_dim
        ctx.out_dim = out_dim
    ...
    @staticmethod
    def backward(
        ctx: "All_to_All_Async", grad_output: Tensor, grad_work=None
    ) -> Tuple[Tensor]:
        ...
        if ctx.in_dim == 2:
            ...
            grad_output = rearrange(
                grad_output, "n (x h) w c -> n h (x w) c", x=mp_size
            )
...
class All_to_All_Async_Opp(torch.autograd.Function):
    ...
    def backward(ctx: "All_to_All_Async_Opp", grad_output: Tensor) -> Tuple[Tensor]:
        global WORLD_WORK_ALL2ALL
        d_tensor, WORLD_WORK_ALL2ALL = _all_to_all_async(
            grad_output, in_dim=ctx.out_dim, out_dim=ctx.in_dim
        )
