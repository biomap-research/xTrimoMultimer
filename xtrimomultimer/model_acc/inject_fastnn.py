...
from .evoformer import EvoformerBlock, ExtraMSABlock
from .triangle import TemplatePairStackBlock
from xtrimomultimer.model_acc.distributed.comm import gather, scatter
...
def copy_qkv_linear(model_fast, ori_q, ori_k, ori_v):
    model_fast.weight.copy_(
        torch.cat((ori_q.weight, ori_k.weight, ori_v.weight), dim=0)
    )


def copy_attention(model_fast, model_ori):
    copy_qkv_linear(
        model_fast.to_qkv, model_ori.linear_q, model_ori.linear_k, model_ori.linear_v
    )
...
def copy_triangle(model_fast, model_ori):
    ...
    copy_left_right(
        model_fast.left_right_projection, model_ori.linear_a_p, model_ori.linear_b_p
    )

    copy_left_right(
        model_fast.left_right_gate, model_ori.linear_a_g, model_ori.linear_b_g
    )
...
def copy_evoformer_para(block_fast, block_ori):
    ...
    copy_attention(
        block_fast.msa_stack.MSAColumnAttention.attention,
    ...
    # communication
    comm_model = (
        block_ori.core.outer_product_mean
        if not block_ori.is_multimer
        else block_ori.outer_product_mean
    )
